...
class UserTest < ActiveSupport::TestCase
  ...
  test "should follow and unfollow a user" do
    dom = users(:dom)
    archer = users(:archer)
    assert_not dom.following?(archer)
    dom.follow(archer)
    assert dom.following?(archer)
    assert archer.followers.include?(dom)
    dom.unfollow(archer)
    assert_not dom.following?(archer)
  end
  ...
  test "feed should have the right posts" do
    ...
    lana.microposts.each do |post_following|
      assert dom.feed.include?(post_following)
    end
    # Posts from self
    dom.microposts.each do |post_self|
      assert dom.feed.include?(post_self)
    end
    # Posts from unfollowed user
    archer.microposts.each do |post_unfollowed|
      assert_not dom.feed.include?(post_unfollowed)
    end
  end
end
